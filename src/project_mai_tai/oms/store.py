...
class OmsStore:
    ...
    def __init__(self) -> None:
        self._open_order_sync_rotation_offset = 0
    ...
    def list_open_orders_for_sync(
        self,
        session: Session,
        *,
        broker_account_ids: list[UUID] | None = None,
    ) -> list[BrokerOrder]:
        orders = self.list_open_orders(session, broker_account_ids=broker_account_ids)
        if len(orders) < 2:
            return orders
        # A fixed newest-first pass made the same old Webull order consume the tail-end 429 on
        # every cycle while newer orders were always served first. Only the synchronization loop
        # advances this cursor; membership checks keep list_open_orders() stable and cannot cancel
        # the rotation by consuming offsets between syncs.
        offset = self._open_order_sync_rotation_offset % len(orders)
        self._open_order_sync_rotation_offset = (offset + 1) % len(orders)
    ...
    # ---------------------------------------------------------------- Webull ineligible (fan-out)
    # Symmetric to the Schwab ineligible cache above; used by the dual-broker fan-out per-broker
    # eligibility gate. A name is only recorded here on a CLEAR not-tradable Webull reject (never on
    # 429 / rate-limit / transient — the classifier in the service excludes those).

    def get_webull_ineligible_entry(
        self,
        session: Session,
        *,
        broker_account_id: UUID,
        symbol: str,
        session_date: str,
    ) -> WebullIneligibleToday | None:
        return session.scalar(
            select(WebullIneligibleToday).where(
                WebullIneligibleToday.broker_account_id == broker_account_id,
                WebullIneligibleToday.symbol == symbol.upper(),
                WebullIneligibleToday.session_date == session_date,
            )
        )

    def record_webull_ineligible_entry(
        self,
        session: Session,
        *,
        broker_account_id: UUID,
        ...
        entry = self.get_webull_ineligible_entry(
            session,
            broker_account_id=broker_account_id,
            symbol=symbol,
            session_date=session_date,
    ...
    def list_webull_ineligible_symbols_by_account(
        self,
        session: Session,
        *,
        broker_account_ids: list[UUID],
        ...
        rows = session.scalars(
            select(WebullIneligibleToday).where(
                WebullIneligibleToday.broker_account_id.in_(broker_account_ids),
                WebullIneligibleToday.session_date == session_date,
            )
        ).all()
        blocked: dict[UUID, set[str]] = {}
