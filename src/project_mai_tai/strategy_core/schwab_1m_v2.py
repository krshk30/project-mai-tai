...
class SymbolState:
    ...
    # Durable fan-out attribution starts at the FIRST fan-out draft, which can precede the BUY-flip
...
class SchwabV2Strategy:
    ...
    def update_position(self, symbol: str, qty: int, *, held_qty: int | None = None) -> None:
        ...
        if prev > 0 and state.position_qty == 0:
            ...
            if self._cw_v2_enabled:
                ...
                # ⛔ Evidence precedence at the release site. The UNION can fall to zero because
                # a Webull fill terminalized its own open intent, while a transient
                # [VIRTUAL-CLEAR] can separately make the fills-only read zero. Both converge
                # here before position restoration can arrive (observed restore lag 6.648–19.119s,
                # versus this transition within 10s in 64/65 reachable primary cases). Neither
                # zero can outrank durable positive Webull order/fill evidence. This protects only
                # the venue-local fan-out claim; v2's own reclaim availability above remains
                # governed by its Schwab composition under operator reading A.
                positive_webull_evidence = (
                    state.fanout_webull_claimed
                    and state.fanout_claim_outcome in {"held", "filled"}
                )
                if positive_webull_evidence:
                    logger.info(
                        "[V2-FANOUT-CLAIM-RELEASE-GUARD] %s trigger=schwab_union_zero "
                        "evaluated=1 release_allowed=0 released=0 held=1 "
                        "evidence_positive=1 outcome=%s spurious=%d — transient zero cannot "
                        "outrank durable Webull evidence",
                        symbol,
                        state.fanout_claim_outcome,
                        int(spurious),
                    )
    ...
    def _ensure_fanout_segment_id(self, state: SymbolState) -> int:
        """Return a stable, non-zero identity shared by every fan-out leg in one segment.

        Resting legs can emit before ``cw_arm_bar_ts`` exists, so that field cannot be the source of
        truth. Bind the identity on the first draft and retain it through the later BUY arm and
        reclaim. Segment-end/session-reset paths clear it. The fallback clock is only used when a
        test/recovery state has neither an arm nor a bar; it still refuses to emit an unattributed
        money-path leg.
        """
        ...
        if segment_id <= 0:
            clock = getattr(self, "_now_ms", None)
            segment_id = max(
                1,
    ...
    def _build_webull_fanout_draft(
        ...
        logger.info(
            "[V2-FANOUT-SEGMENT-BOUND] %s source=%s trigger=fanout_draft attributed=1 "
            "segment_id=%d entry_n=%d — polarity: attributed=1 means this leg can be grouped; "
