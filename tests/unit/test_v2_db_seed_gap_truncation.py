...
class _Strategy:
    ...
    def __init__(self):
        ...
        self._st = SimpleNamespace(
            pending_path_macd=False, pending_path_vwap=False,
            pending_cross_bar_ts_ms=0, bars=self.seen,
        )
    def on_bar(self, _symbol, bar): self.seen.append(bar)
    def watchlist_state(self, _symbol): return self._st


def _seed(rows: list[_Row], sessions: set | None = None):
...
def test_CAST_shape_the_june_bars_never_reach_the_strategy() -> None:
    ...
    strat, bot = _seed(_series(today, 38) + _series(june, 212),
...
def test_the_dropped_side_is_what_produced_the_6_7x_error() -> None:
    ...
    today = datetime(2026, 8, 18, 11, 0, tzinfo=UTC)
    june = datetime(2026, 6, 18, 18, 49, tzinfo=UTC)
    strat, _ = _seed(_series(today, 38, close=1.21) + _series(june, 212, close=7.99),
                     sessions={date(2026, 7, 1)})
...
def test_truncation_stops_at_the_FIRST_gap_not_the_widest() -> None:
    ...
    rows = _series(now, 10) + _series(now - timedelta(days=30), 10) + _series(now - timedelta(days=31), 10)
