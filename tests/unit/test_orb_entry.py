...
def test_breakout_emits_open_intent_for_universe_name():
    svc = _svc(["CRVO"])
    _feed_or(svc, "CRVO")
    assert _pending(svc) == []  # no breakout during the OR window
...
def test_name_not_in_universe_is_skipped():
    ...
    _feed_or(svc, "CRVO")
    svc._on_bar("CRVO", _bar(5, 5.1, 5.4, 5.05, 5.33, 300, vwap=5.1, ema9=5.05))
    assert _pending(svc) == []  # not armed — out of scope
...
def test_intrabar_mode_fills_at_or_high():
    ...
    assert _pending(svc) == [("X", 5.09)]  # OR_high, not the close


def test_paper_decision_metadata_records_trail8():
    svc = _svc(["X"])
    event = svc._build_paper_entry_decision("X", 5.33, observed_at=OPEN)
    assert event.event_type == "PAPER_ENTRY_DECISION"
    assert int(event.quantity) == 10
    md = event.detail["metadata"]
    assert md["stop_guard_enabled"] == "true"
    assert md["stop_loss_pct"] == "8.0"   # initial stop 8% below entry
    assert md["trail_pct"] == "8.0"
