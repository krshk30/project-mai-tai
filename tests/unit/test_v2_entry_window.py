"""v2 trading-window entry gate — the operator's 7 AM–6 PM ET rule.
...
(default 7–18). Exits (cw_flip + OMS-managed) are unaffected.
"""
...
# --- _within_entry_window boundaries (default 7–18 ET) ---
...
def test_within_entry_window_inside() -> None:
    ...
    assert svc._within_entry_window(datetime(2026, 7, 14, 17, 59, tzinfo=EASTERN)) is True  # 5:59 PM
...
def test_within_entry_window_outside() -> None:
    ...
    assert svc._within_entry_window(datetime(2026, 7, 14, 18, 0, tzinfo=EASTERN)) is False   # 6 PM sharp
