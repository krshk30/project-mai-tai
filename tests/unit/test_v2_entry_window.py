...
def test_default_entry_window_is_seven_to_sixteen_hundred() -> None:
    ...
    2026-07-24 Phase A: the end is 16:00 (was 16:30). Phase B: the start is 07:00 (operator 07-24 —
    "7 AM is the pre-market open"; revised from the interim 07:30). This is the threshold-pinning test:
    mutate the start-minute (0->30) or end-minute default and it turns red."""
    s = Settings()
...
def test_within_entry_window_inside() -> None:
    svc = _svc()
    assert svc._within_entry_window(datetime(2026, 7, 14, 7, 0, tzinfo=EASTERN)) is True    # 7:00 AM sharp (Phase B start)
...
def test_within_entry_window_outside() -> None:
    svc = _svc()
    assert svc._within_entry_window(datetime(2026, 7, 14, 6, 59, tzinfo=EASTERN)) is False   # 6:59 — one minute before the 07:00 start
    assert svc._within_entry_window(datetime(2026, 7, 14, 6, 0, tzinfo=EASTERN)) is False    # pre-7 AM
