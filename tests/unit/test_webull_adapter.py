...
@pytest.mark.asyncio
async def test_filled_oco_leg_without_broker_fill_time_cannot_be_attributed(fake_sdk) -> None:
    body = _leg("FILLED", "1.9500", oid="WB-WETO-STOP")
    del body["items"][0]["last_filled_time"]
    client = _LegClient({_BASE + "S": body})

    with pytest.raises(ValueError, match="missing broker fill time"):
        await _adapter(client).fetch_oco_exit_fill("live:orb", "WETO", _BASE)
    assert client.seen == [_BASE + "T", _BASE + "S"]
