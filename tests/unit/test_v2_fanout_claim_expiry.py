...
def test_the_claim_is_timestamped_when_taken() -> None:
    ...
    assert "state.fanout_webull_claimed = True" in src
    assert "state.fanout_claim_ms = self._now_ms()" in src
...
def test_an_unfilled_claim_is_RELEASED_after_the_grace() -> None:
    ...
    src = inspect.getsource(strat.SchwabV2Strategy._fanout_rth_resting_cross)
    assert "state.fanout_webull_claimed = False" in src
...
def test_release_requires_NO_position() -> None:
    ...
    src = inspect.getsource(strat.SchwabV2Strategy._fanout_rth_resting_cross)
    head = src.split("state.fanout_webull_claimed = False")[0]
...
def test_release_happens_BEFORE_the_claim_gate() -> None:
    ...
    src = inspect.getsource(strat.SchwabV2Strategy._fanout_rth_resting_cross)
    release = src.index("state.fanout_webull_claimed = False")
...
def test_every_release_site_also_clears_the_timestamp() -> None:
    ...
    sets = src.count("state.fanout_webull_claimed = False")
    stamps = src.count("state.fanout_claim_ms = 0")
    assert stamps >= sets, f"{sets} release sites but only {stamps} timestamp clears"
