...
def test_schwab_native_entry_engine_blocks_consecutive_prev_bar_p4_rearm_on_same_move() -> None:
    config = TradingConfig().make_30s_schwab_native_variant()
    engine = SchwabNativeEntryEngine(
        config,
        now_provider=lambda: datetime(2026, 4, 17, 10, 0),
    ...
    first_signal = engine.check_entry(
        "ELAB",
        {
            "open": 2.17,
            "price": 2.205,
    ...
    assert first_signal["path"] == "P4_BURST"

    second_signal = engine.check_entry(
        "ELAB",
        {
            "open": 2.20,
            "price": 2.34,
