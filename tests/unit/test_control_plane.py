...
def test_control_plane_marks_no_exposure_data_halt_placeholder_blocked() -> None:
    settings = Settings(redis_stream_prefix="test", oms_adapter="alpaca_paper")
    session_factory = build_test_session_factory()
    seed_database(session_factory)
    streams = make_streams(settings.redis_stream_prefix)
    ...
    runtime_bot.data_health = {
        ...
        "halted_symbols": ["AUUD"],
        "reasons": {
            "AUUD": "Completed bar flow stalled: fresh POLYGON ticks are arriving but the last completed 30s bar is 180.0s old; new entries halted until bar flow recovers"
        },
        "since": {"AUUD": "2026-04-23 10:48:17 AM ET"},
    }
    strategy_state_stream[0][1]["data"] = strategy_state_event.model_dump_json()
    redis = FakeRedis(streams)
    ...
    with TestClient(app) as client:
        ...
        assert bots.status_code == 200
        bot_30s = next(item for item in bots.json()["bots"] if item["strategy_code"] == "macd_30s")
        assert bot_30s["recent_decisions"][0]["status"] == "blocked"
...
def test_control_plane_decision_tape_escalates_stalled_completed_bar_wait(
    ...
    with TestClient(app) as client:
        ...
        bot_30s = next(item for item in bots.json()["bots"] if item["strategy_code"] == "macd_30s")
        assert [item["symbol"] for item in bot_30s["recent_decisions"]] == ["AUUD"]
        assert bot_30s["recent_decisions"][0]["status"] == "critical"
        assert (
            bot_30s["recent_decisions"][0]["reason"]
            == "live in bot; no completed 30s trade bar for 1m33s after the last live Polygon tick - verify tape/bar flow now"
        )
