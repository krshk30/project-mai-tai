...
#
# WINDOW: page from 06:00-16:30 ET. The first hour is deliberate pre-entry warning before v2's
# 07:00 ET entry window opens; after that, all three faults can suppress or endanger live entries.
# Every 5 min (matches the F3 fleet-health cadence).
#
# CRON_TZ IS IGNORED ON THIS BOX (learned the hard way — see the pre-open readiness infra). So the
# cron runs over a UTC hour range wide enough to cover BOTH offsets (EDT 10:00-20:30, EST
# 11:00-21:30 UTC) and the ET guard below does the precision:
#   */5 10-21 * * 1-5   /home/trader/project-mai-tai/ops/health/armed_segments_cron.sh
#
...
# --- ET guard: 06:00 (360) <= ET < 16:30 (990), Mon-Fri ---
if [ "$ETMIN" -lt 360 ] || [ "$ETMIN" -ge 990 ]; then
  exit 0   # outside the pager window — quiet, not even logged
fi
