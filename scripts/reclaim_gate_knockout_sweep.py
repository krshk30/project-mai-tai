...
KNOCKOUTS: dict[str, dict[str, object]] = {
    "baseline": {},
    "pullback_off": {
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run one-by-one reclaim gate knockout sweeps.")
    parser.add_argument("--profiles", nargs="*", default=list(KNOCKOUTS), help="Profile names to run.")
    return parser.parse_args()


def main() -> None:
    args = _parse_args()
    selected = [name for name in args.profiles if name in KNOCKOUTS]
    if not selected:
        raise SystemExit("No valid profiles requested.")

    results: list[dict[str, object]] = []
    output_path = SCRIPT_DIR.parent / "tmp_replay" / "reclaim_gate_knockout_sweep.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    for profile_name in selected:
        overrides = KNOCKOUTS[profile_name]
