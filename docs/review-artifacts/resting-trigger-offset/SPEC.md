...
**Written by `claude-1` 2026-09-22 ~18:40 ET for `codex-2` to build and review. Nothing built yet.**
...
- Logs: every `[V2-RESTING-PLACED]` / reprice line prints `offset_pct=` next to `band_pct=` so tomorrow's tape can be
  read without inferring.
