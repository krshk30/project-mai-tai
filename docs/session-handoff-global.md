...
1. **[P1 — BLOCKER for trading, NOT for validation] v2 execution path: route to PAPER/SIMULATED, do NOT give it the shared Schwab hash.**
   ...
   - **DESIGN DONE — TOP URGENCY (operator 2026-06-09: outranks even P0).** `docs/p1-v2-deliberate-paper-routing-design.md`. Make v2's paper-ness deliberate/structural so it CANNOT reach the real account even if the `configured_schwab_accounts` entry is added. **Phase 1 (now, safe before P0):** flip `strategy_schwab_1m_v2_broker_provider` default `"schwab"`→`"simulated"` (routes v2's orders to `SimulatedBrokerAdapter` — which DOES exercise fills, so execution finally gets validated: order→fill→position→P&L) + a hash-side guard (`configured_schwab_accounts` refuses `paper:` accounts). Phase 1 is **v2-scoped → does NOT disturb P0** (retired bots stay `provider=schwab`, keep the incidental token refresher alive). **Phase 2 (with/after P0):** full `paper:`→sim enforcement — DEFERRED because forcing *all* `paper:`+schwab accounts to sim removes the last schwab-provider account → drops the incidental `SchwabBrokerAdapter` → triggers the P0 token SPOF. ⚠️ Sequencing: P1-Phase-1 first; Phase-2 only once P0's dedicated refresher owns token freshness. Go-live ceremony = rename `paper:`→`live:` + provider→schwab + add hash, attended.
