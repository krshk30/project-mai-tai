...
1. **[P1 — BLOCKER for trading, NOT for validation] v2 execution path: route to PAPER/SIMULATED, do NOT give it the shared Schwab hash.**
   ...
   - **DESIGN DONE — TOP URGENCY (operator 2026-06-09: outranks even P0); sequencing APPROVED: P1-Phase-1 → P0 → P1-Phase-2.** `docs/p1-v2-deliberate-paper-routing-design.md`. Make v2's paper-ness deliberate/structural so it CANNOT reach the real account even if the `configured_schwab_accounts` entry is added. **Phase 1 (now, safe before P0) — both layers V2-SCOPED:** (a) flip `strategy_schwab_1m_v2_broker_provider` default `"schwab"`→`"simulated"` (routes v2's orders to `SimulatedBrokerAdapter`) + (b) hash-guard refusing **`paper:schwab_1m_v2` specifically** (NOT all paper: accounts). Recommendations for implement-review: sink = `SimulatedBrokerAdapter`; mechanism = default-flip (+ explicit VPS env). **Phase 2 (with/after P0):** broaden BOTH routing AND hash-guard to all `paper:` accounts — DEFERRED because each removes the retired bots' schwab footprint that triggers the incidental token refresh → P0 SPOF. ⚠️ Confirmed: `list_account_positions` returns `[]` with no REST call when an account isn't in `accounts_by_name`, so a broad hash-guard (removing retired bots) kills the refresh trigger too — hence both broad changes wait for P0. Go-live ceremony = rename `paper:`→`live:` + provider→schwab + add hash, attended.
