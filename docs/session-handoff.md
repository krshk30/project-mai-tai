...
**Written by `claude-1`, 2026-09-24 (Thu) 08:22 ET (the close-out ran the next morning; every box read in it is stamped with its own time).** Batch `2026-09-23-five-flags-one-restart-whlr-dead-exit-benf-sparse-bars`.
...
| **flags — ALL ON; FOUR newly enabled tonight + one code fix** | From `/proc/<pid>/environ`, oms AND v2, 20:05 ET: `RESTING_TRIGGER_OFFSET_PCT=0.5` (#1035) · `RTH_EDGE_BRACKET_ENABLED=true` (C, #647 — **never exercised**) · `GAP_HOLD_ENABLED=true` (#1038) · `RETRY_ONE_ENABLED=true` (#1041, max_retries 1) · `LATE_CLOSE_GUARD_ENABLED=true` (LC1, since 09-22) · `MIRROR_DEFERRED_RESUBMIT_ENABLED=true` (PA1, proven). ⛔ **FIVE changes in ONE restart by the operator's word (four new flags + the #1040 code fix)** — tomorrow is graded by one marker per change (table below) so the suspects stay separable |
| Webull size | 1 share |
...
|---|---|---|---|
| **oms** | **32368** | Wed 09-23 19:50:34 | deploy `e0eb8831` + four flags |
| **strategy** | **32379** | Wed 09-23 19:50:34 | same |
| **schwab-1m-v2** | **33096** | Wed 09-23 19:52:45 | same (v2 reads the offset / gap-hold / retry flags) |
...
## What is LIVE and what to READ Thursday 09-24 (owner · first read) — report UNPROMPTED, one marker per change
...
- **Momentum:** #1029 still draft; parser fixed (`ac91ea20`). **The 09-23 detached Step 2 run ABORTED at 16:25:41 ET: rc=2
  UNMEASURED (one-minute load 3.697 > the frozen 3.5 ceiling), zero frames, no valid replay result.** Not an ongoing capture;
  a new ten-minute baseline is required before any retry (`codex-2`).
...
| **Momentum #1029** | codex-2 | 09-23 replay ABORTED rc=2 UNMEASURED (load 3.697 > 3.5); retry only from a fresh ten-minute baseline in a flat 16:05–20:00 ET window; `claude-1` reviews, reading the CONTROL first |
...
| TREND1 / 30-min / spike patterns | operator | SUSPENDED by the operator 09-22 ("we are chasing the downtrend, retry, everything — suspend"); results in memory; reopens only on his word |
