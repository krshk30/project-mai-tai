...
- **🟢 ORB = LIVE real-money (2026-06-24 night) → PID 2765863** (18:00 ET start). Running-high mode, `live:orb`→**webull margin** (D4GUJ…), qty 5, 3% trail, 9:30–10:00 ET window, 1.5% gap-cap. Plumbing proven green (buy→`[HARD-STOP ARMED]`→sell→`[HARD-STOP CLEARED]`→flat on real AZI fills). ⚠️ **restart-while-holding UNTESTED — don't restart OMS while ORB holds.** Dashboard shows ORB provider "alpaca" (display-only `active_broker_providers` cosmetic; routing is webull). **OMS restarted tonight → PID 2765200** (4 Webull fixes #374–#377 + #373).
