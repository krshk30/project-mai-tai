...
- **🆕 V2 ATR hold-confirm Path-B LEAK — DECISION PENDING (measured 2026-06-24, not yet built).** Hold-confirm IS live/enabled (N=20s/5bps), but **83% of actual entries (5/6 in the 06-23/24 era; 100% historically) leak through the UNCONFIRMED bar-close fallback (Path B)**, and Path-B is net-negative (live −$4.89/32%win; backtests −$5.91 & −$18.78/~15%win) — i.e. the bar-close fallback undoes the hold-confirm edge. **Decide: (1) apply the 20s net_delta confirm to Path-B too, or (2) skip bar-close-only flips entirely** (backtest option 2 first — dropping Path-B may itself flip net positive). Deeper: the ATR ENTRY EDGE (~15% win, buys faders that reverse) is the real weak link, NOT the watchlist — the change%<30 scanner-fade rule (#277) is net-PROTECTIVE for ATR, keep as-is. [[project_mai_tai_v2_atr_validation]]
...
- **🟢 FCUV manual-position conflict — VERIFIED SAFE (06-25, do NOT protect).** `live:orb` (webull) holds **400 sh FCUV @ $6.87** (operator's MANUAL position; no ORB order created it) and FCUV is on ORB's watchlist. Operator trades FCUV by hand and chose to leave it **unprotected/tradeable** (`MAI_TAI_PROTECTED_SYMBOLS=CYN` only). **Code-verified the OMS will NOT touch it:** `oms_managed_positions` has a single writer gated to `schwab_1m_v2` only; ORB exits run off the OMS native hard-stop, which arms **only** on a fill from an intent ORB emitted (`_armed_hard_stops[key]` must pre-exist) — armed stops are in-memory, empty on restart, re-armed only from new bot fills. Reconciler will emit a benign position-mismatch finding for FCUV (like CYN). If ORB enters FCUV today it adds its own qty-10 managed leg; its exit sells only the managed qty, leaving the manual 400. **FILL-FEED CROSS-TALK also verified SAFE (06-25):** `sync_broker_state` polls **only `list_open_orders`** (OMS-created rows) via `fetch_order_update` keyed on the bot's own `client_order_id`; a manual sell has no OMS order row → never fetched, never recorded as a Fill, never reaches `apply_fill_to_positions`/hard-stop registry (fills dedup on `broker_fill_id`). Broker AGGREGATE position is read only as (a) a sell-sizing cap `min(intent.qty, virtual.qty, remaining_acct)` and (b) a backing check that zeros a virtual leg only when broker shows **0** shares. So a manual FCUV sell cannot misattribute to / clear / mis-track ORB's leg. **Residual risk = share fungibility only** (shared account = one pool; overselling below ORB's qty while it holds could eat ORB's shares → stop rejects, leg zeroed next sync). **RESOLUTION 06-25: operator will NOT manually trade FCUV in Webull today** → no manual sells → coexistence clean; FCUV left unprotected (no OMS restart). NOTE: ORB does NOT read protected_symbols itself; the working exclusion lever is the OMS `_evaluate_risk` gate (`MAI_TAI_PROTECTED_SYMBOLS` → rejects any bot intent for the symbol; no effect on manual trading) + OMS restart.
